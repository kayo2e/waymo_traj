...
# 현재 폴더 안의 'libs' 폴더를 라이브러리 탐색 1순위로 지정!
...
## GameFormer 비최적 모드 색상 팔레트
MODE_COLORS_GF = ["#ff9999", "#ffcc99", "#ffff99", "#99ff99", "#99ccff"]
...
def parse_args():
    ...
    ## GameFormer 코드 작성
    p.add_argument("--gf_result", type=str, default=None,
                   help="GameFormer result.pkl 파일 경로")
...
def load_gameformer_results(path):
    """GameFormer.pkl 결과 로드: {scenario_id: predicted_trajectories} 딕셔너리 반환"""
    import pickle
    import numpy as np

    if not path or not os.path.exists(path):
        print(f"[GameFormer] 결과 파일 없음: {path}")
        return None
    with open(path, "rb") as f:
        data = pickle.load(f)
    ...
    if isinstance(data, list):
        for i, item in enumerate(data):
            # 1. 시나리오 ID 추출 (출력값에 ID가 없다면 임시 ID 부여)
            sc_id = item.get('scenario_id', f"unknown_idx_{i}")

            # 2. 궤적 데이터 추출 및 Numpy 변환
            pred = np.array(item['predicted_trajectories'])

            # 3. 5차원 [1, 1, 6, 80, 2] -> 3차원 [6, 80, 2] (Ego 차량 데이터만 추출)
            if pred.ndim >= 3:
                # 맨 뒤의 3개 차원(K, T, C)만 남기고 다 합친 뒤, 첫 번째(0) 객체만 가져옴
                K, T, C = pred.shape[-3:]
                pred = pred.reshape(-1, K, T, C)[0]

            by_id[sc_id] = pred
    else:
        by_id = data

    print(f"[GameFormer] {len(by_id)}개 시나리오 로드 완료")
    return by_id

def mtr_pred_to_ego(pred_trajs, x0, y0, cos_h, sin_h): ## 여긴 GameFormer껄 추가해야 할지 기존 걸 쓸지 감이 잘 안옴
...
def get_mtr_pred(sc, mtr_by_id): ## QCNet껄 추가해야 되지만, 코드가 잘 이해가 안 됨.너무 어려움.
...
def main():
    ...
    ## ── GameFormer 결과 로드 ─────────────────────────────────────────────────────────
    gf_by_id = load_gameformer_results(args.gf_result)
    # ── 모델 로드 ────────────────────────────────────────────────────────────
    ...
    ## GameFormer 예측
        ...
        gf_pred = None
        bk_gf = None
        if gf_by_id is not None:
            # 1. 현재 TFRecord의 sc_id와 일치하는 결과가 있는지 확인
            if sc_id in gf_by_id:
                gf_pred = gf_by_id[sc_id]

            # 2. pkl에 ID가 없어서 unknown으로 들어간 경우 (테스트 런), 첫 번째 결과를 강제 매핑
            elif len(gf_by_id) > 0 and list(gf_by_id.keys())[0].startswith("unknown_idx_"):
                first_key = list(gf_by_id.keys())[0]
                gf_pred = gf_by_id[first_key]
                # print(f"  [GameFormer] 주의: pkl에 ID가 없어 임의 매핑합니다.")

            # (K, 80, 2) 차원이 확보되었으므로 정상적으로 오차 계산 진행
            if gf_pred is not None:
                bk_gf = best_mode_idx(gf_pred, gt_traj, gt_valid)
                gf_ade, gf_fde = compute_minADE_FDE(gf_pred, gt_traj, gt_valid)
    ...
    for ax, is_zoom in [(ax_full, False), (ax_zoom, True)]:
        ...
        # --- GameFormer 시각화 추가 ---
        if gf_pred is not None:
            K_gf = gf_pred.shape[0]
            gf_color = "#00ced1"  # DarkTurquoise (청록색)

            # 1. 비최적 모드 (흐린 점선)
            for k in range(K_gf):
                if k == bk_gf: continue
                # 단일 색상(gf_color) 대신 미리 정의한 팔레트 사용
                col = MODE_COLORS_GF[(k % len(MODE_COLORS_GF))]
                ax.plot(gf_pred[k][valid_idx, 0], gf_pred[k][valid_idx, 1],
                        color=col, linewidth=1.2, linestyle=":", alpha=0.6, zorder=4)

            # 2. 최적 모드 (굵은 실선)
            ax.plot(gf_pred[bk_gf][valid_idx, 0], gf_pred[bk_gf][valid_idx, 1],
                    color=gf_color, linewidth=2.5, linestyle="-",
                    label=f"GameFormer best  minADE={gf_ade:.2f}m", zorder=6)
            add_time_ticks(ax, gf_pred[bk_gf], valid_idx, gf_color, zorder=7)
            add_arrow(ax, gf_pred[bk_gf], valid_idx, gf_color, zorder=7)
        ...
        if is_zoom:
            ...
            if gf_pred is not None:
                extra_patches.append(
                    mpatches.Patch(color="#00ced1", alpha=0.4,
                                   label=f"GF other {K_gf - 1} modes (dotted)")
                )
    ...
    ## print 추가
    if gf_pred is not None:
        print(f"  GF  minADE={gf_ade:.3f}m  minFDE={gf_fde:.3f}m  (best mode={bk_gf})")
    else:
        print("  GF  N/A (시나리오가 GameFormer 결과에 없음)")
