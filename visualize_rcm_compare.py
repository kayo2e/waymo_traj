"""
기존 RCM vs 수정 RCM (traj_fix) 궤적 예측 비교 시각화.

동일한 시나리오에서 두 모델의 K=6 예측을 나란히 표시.
  - RCM (baseline): mean-pool, no time_emb, no vel_proj
  - RCM (traj_fix): last-token + time_emb + vel_proj  ★

Run:
    python visualize_rcm_compare.py
    python visualize_rcm_compare.py --scenario_idx 3
    python visualize_rcm_compare.py --curve
    python visualize_rcm_compare.py \
        --ckpt_base checkpoints/baseline_quick/model_best.pt \
        --ckpt_fix  checkpoints/traj_fix/model_best.pt
"""
...
def parse_args():
    ...
    p.add_argument("--scenario_id",   type=str, default=None,
                   help="특정 scenario_id를 직접 지정 (지정 시 idx/curve 무시)")
...
def load_rcm(ckpt_path, use_traj_fix, device, use_map_per_step=False):
    ...
    if os.path.exists(ckpt_path):
        ...
        label = f"RCM ep{ep}  (val ADE={ade:.3f}m)"
...
def best_k(pred, gt, valid):
    vi = np.where(valid)[0]
    if len(vi) == 0:
        return 0
    ades = [np.linalg.norm(pred[k][vi] - gt[vi], axis=1).mean()
...
def draw_model(ax, pred, gt, valid_idx, color_best, colors_other,
               label_best, is_zoom, zorder_base=4):
    K = pred.shape[0]
    bk = best_k(pred, gt, np.zeros(gt.shape[0], bool))
    # valid_idx 사용하여 bk 재계산
    vi = valid_idx
    ades = [np.linalg.norm(pred[k][vi] - gt[vi], axis=1).mean()
            for k in range(K)]
    bk = int(np.argmin(ades))
...
def main():
    ...
    with torch.no_grad():
        pred_base = model_base(ego_h, soc, map_s, traf,
                               risk_label=risk_gt, gt_traj=None
                               )["trajectory"][0].cpu().numpy()  # [K,80,2]
    ...
    bk_base = int(np.argmin([
        np.linalg.norm(pred_base[k][vi] - gt_traj[vi], axis=1).mean()
        for k in range(pred_base.shape[0])
    ]))
    bk_fix = int(np.argmin([
        np.linalg.norm(pred_fix[k][vi] - gt_traj[vi], axis=1).mean()
        for k in range(pred_fix.shape[0])
    ]))

    # ── 줌 범위 ──────────────────────────────────────────────────────────────
    zx0, zx1, zy0, zy1 = compute_bounds([
        gt_traj[vi], ego_hist_xy,
        pred_base[bk_base][vi], pred_fix[bk_fix][vi],
    ], margin=12)

    # ── 캔버스 ───────────────────────────────────────────────────────────────
    fig, (ax_full, ax_zoom) = plt.subplots(1, 2, figsize=(18, 8))
    fig.suptitle(
        f"RCM Comparison  |  Scenario: {sc_id}\n"
        f"ego-relative coords  (T=11 hist → 80 future @ 10 Hz = 8 s)",
        fontsize=12, y=1.01
    )

    for ax, is_zoom in [(ax_full, False), (ax_zoom, True)]:
        draw_map(ax, feats["scene_tensor"],
                 alpha=0.45 if is_zoom else 0.3)
        ...
        draw_model(ax, pred_base, gt_traj, vi,
                   ...
                   f"RCM baseline  minADE={ade_base:.2f}m  minFDE={fde_base:.2f}m",
                   is_zoom, zorder_base=4)

        # RCM traj_fix (빨간 계열)
        draw_model(ax, pred_fix, gt_traj, vi,
                   COLOR_FIX_BEST, COLORS_FIX_OTHER,
                   f"RCM traj_fix★  minADE={ade_fix:.2f}m  minFDE={fde_fix:.2f}m",
                   is_zoom, zorder_base=5)
    ...
    plt.tight_layout()
    out_path = os.path.join(args.out_dir, f"rcm_compare_{sc_id}.png")
